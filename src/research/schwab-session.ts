...
// auto-detected: SCHWAB_CLIENT_ID and SCHWAB_CLIENT_SECRET in the environment
// or .env, plus tokens in data/oauth-tokens.json from `npm run auth`.
...
export async function schwabSession(provider: ProviderChoice): Promise<SchwabSession> {
  ...
  if (!clientId || !clientSecret) {
    const reason = "SCHWAB_CLIENT_ID / SCHWAB_CLIENT_SECRET not set (environment or .env in this directory)";
  ...
  return { rest: new SchwabRest(auth), reason: "credentials and tokens found" };
}
