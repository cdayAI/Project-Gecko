...
| `webull` | real-time with subscription, sandbox delayed | yes | yes (gainers, losers, most active, premarket) | contract discovery + snapshots in batches of 20; needs OPRA non-display subscription | yes | `WEBULL_APP_KEY`, `WEBULL_APP_SECRET` |
...
|---|---|---|
| Token config | `GET /openapi/config` | `token_check_enabled` |
| Token create / check | `POST /auth/tokens/create`, `POST /auth/tokens/check` | 2FA approval in the Webull app; verified token cached in `data/webull-token.json` (mode 0600) |
| Stock/ETF snapshot | `GET /market-data/stocks/snapshots/list?symbols=&category=US_STOCK|US_ETF&extend_hour_required=` | up to 100 symbols |
| Depth quotes | `GET /market-data/stocks/depths/list?symbol=&category=&depth=1` | |
...
| Option contracts | `GET /trading/instruments/options/contracts/list?category=US_OPTION&underlying_symbols=` | response shape not yet observed; OCC symbols are extracted from any string field |
| Option snapshot | `GET /market-data/options/snapshots/list?symbols=&category=US_OPTION` | 20 symbols per call, 60 calls/min; needs OPRA non-display |
...
need OPRA Real-Time Non-display. Sandbox (`WEBULL_ENV=sandbox`) serves
15-minute delayed data. Rate limit 300 requests per minute on the HTTP API.
...
4. Add chains back. The first option call logs the raw top-level keys so the
   parser can be confirmed against the real shape.
